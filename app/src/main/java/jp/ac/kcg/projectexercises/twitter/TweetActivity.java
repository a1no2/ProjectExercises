package jp.ac.kcg.projectexercises.twitter;
...
import jp.ac.kcg.projectexercises.main.GlobalKt;
...
import jp.ac.kcg.projectexercises.twitter.tweet.TweetService;
import kotlin.Unit;
...
public class TweetActivity extends SubsidiaryActivity implements TextWatcher {
    ...
    private static ClientUser currentClientUser = null;
    ...
    public static void replyTweet(Context activityContext, ClientUser clientUser, Tweet destinationTweetStatus) {
        if (!(activityContext instanceof ApplicationActivity)) return;
        currentClientUser = clientUser;
        destinationUserNames.addAll(destinationTweetStatus.getMentionEntityUserNames());
        destinationStatusText = destinationTweetStatus.getUser().getScreenName() + " : " + destinationTweetStatus.getText();
        destinationStatusId = destinationTweetStatus.getId();
        ((ApplicationActivity) activityContext).startActivity(TweetActivity.class, false);
    }

    public static void quoteRetweet(Context activityContext, ClientUser clientUser, Tweet quotedTweetStatus) {
        if (!(activityContext instanceof ApplicationActivity)) return;
        currentClientUser = clientUser;
        tweetText = "QT " + quotedTweetStatus.getUser().getScreenName() + " >" + quotedTweetStatus.getText() + " : ";
        ((ApplicationActivity) activityContext).startActivity(TweetActivity.class, false);
    }

    public static void hashtagTweet(Context activityContext, ClientUser clientUser, String hashtag) {
        if (!(activityContext instanceof ApplicationActivity)) return;
        currentClientUser = clientUser;
        tweetText = hashtag + " ";
        ((ApplicationActivity) activityContext).startActivity(TweetActivity.class, false);
    ...
    public static void hashtagsTweet(Context activityContext, ClientUser clientUser, List<String> hashtags) {
        if (!(activityContext instanceof ApplicationActivity)) return;
        currentClientUser = clientUser;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            ClientUsers.Companion.getInstance().loadUsers((clientUsers, success) -> {
                if (!success) {
                    finish();
                    return Unit.INSTANCE;
                }
                currentClientUser = currentClientUser == null ? clientUsers.get(0) : currentClientUser;
                String currentTweetUserName = currentClientUser == null ? clientUsers.get(0).getScreenName() : currentClientUser.getScreenName();
                ...
                tweetButton.setOnClickListener(v -> sendTweet(currentClientUser));
    ...
    private void sendTweet(ClientUser clientUser) {

        TweetBuilder tweetBuilder = clientUser.getTweetBuilder();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (bitmap != null) {
            try {
                ...
                float y = GlobalKt.getWindowSize(this).y / 9;
                float magnification;
            ...
            sendToast("画像を取得できませんでした。");
            Log.v("TweetActivity", "Check path:" + uri);
        }
    }

    private Bitmap geImage(Intent data) {
    ...
    private void removeAppendedImage(View image) {
        AlertDialog alertDialog = new AlertDialog.Builder(this).setMessage("選択した画像を投稿予定から除外しますか？")
    ...
    private void createChangeUserDialog() {
        ClientUsers.Companion.getInstance().loadUsers((clientUsers, success) -> {
            List<CharSequence> ClientUsersName = new ArrayList<>();
            List<ClientUser> ClientUsers = new ArrayList<>();
            for (ClientUser ClientUser : clientUsers) {
                ClientUsersName.add(ClientUser.getScreenName());
                ClientUsers.add(ClientUser);
            }
            AlertDialog alertDialog = new AlertDialog.Builder(this).setItems(ClientUsersName.toArray(new CharSequence[ClientUsersName.size()]), (dialog, which) -> {
                currentClientUser = ClientUsers.get(which);
                currentUserTextView.setText(ClientUsersName.get(which));
            }).create();
            showDialog(alertDialog);
            return Unit.INSTANCE;
        });
